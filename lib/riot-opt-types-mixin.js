...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	function validateOpts(optTypes, opts, tagName) {
	    ...
	    for (var key in opts) {
	        if (!optTypes.hasOwnProperty(key) && key !== 'dataIs' && key !== 'optTypes' && key !== 'riotTag') {
	            appendError(new Error('Opt `' + key + '` was passed to tag `' + tagName + '`, but was not defined in `optTypes` object.'));
	        }
	    }
	    ...
	    for (var _key in optTypes) {
	        if (optTypes.hasOwnProperty(_key)) {
	            var error = optTypes[_key](opts, _key, tagName);
	...
	exports.default = {
	    init: function init() {
	        ...
	        if (!this.optTypes) {
	            appendError(new Error('The `optTypes` object was not set in the tag `' + this.opts.dataIs + '` ' + 'and is expected when using the mixin riot-opt-types-mixin.'));
	        }

	        // validate all opts of the tag on mount and updated
	        if (this.optTypes) {
	            this.on('update', function () {
	                validateOpts(_this.optTypes, _this.opts, _this.opts.dataIs);
	            });
	        }
