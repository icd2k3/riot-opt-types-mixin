	        ...
	        var tagName = this.opts.dataIs,
	            ...
	            for (var _key in _this.optTypes) {
	                if (_this.optTypes.hasOwnProperty(_key)) {
	                    var error = _this.optTypes[_key](_this.opts, _key, tagName, 'prop');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getIteratorFn = __webpack_require__(3);
	...
	var ReactPropTypes = {
	  ...
	  /* element: createElementTypeCheckerwas removed by riot-opt-types-mixin webpack config */
	...
	function createArrayOfTypeChecker(typeChecker) {
	  function validate(opts, optName, componentName, location, optFullName) {
	    if (typeof typeChecker !== 'function') {
	      return new Error('Property `' + optFullName + '` of component `' + componentName + '` has invalid PropType notation inside arrayOf.');
	    }
	    var optValue = opts[optName];
	...
	/* function createElementTypeCheckerwas removed by riot-opt-types-mixin webpack config */
	...
	function createObjectOfTypeChecker(typeChecker) {
	  function validate(opts, optName, componentName, location, optFullName) {
	    if (typeof typeChecker !== 'function') {
	      return new Error('Property `' + optFullName + '` of component `' + componentName + '` has invalid PropType notation inside objectOf.');
	    }
	    var optValue = opts[optName];
	    var optType = getPropType(optValue);
	    if (optType !== 'object') {
	...
	function createNodeChecker() {
	  function validate(opts, optName, componentName, location, optFullName) {
	    if (!isNode(opts[optName])) {
	      var locationName = 'opt' /* ReactPropTypeLocationNames[location]was replaced by 'opt' in riot-opt-types-mixin webpack config */;
	      return new Error('Invalid ' + locationName + ' `' + optFullName + '` supplied to ' + ('`' + componentName + '`, expected a ReactNode.'));
	    }
	    return null;
	  }
	  return createChainableTypeChecker(validate);
	...
	function isNode(optValue) {
	  switch (typeof optValue) {
	    ...
	    case 'object':
	      if (Array.isArray(optValue)) {
	        return optValue.every(isNode);
	      }
	      if (optValue === null/* || ReactElement.isValidElement(optValue)was removed by riot-opt-types-mixin webpack config */) {
	        return true;
	      }

	      var iteratorFn = getIteratorFn(optValue);
	      if (iteratorFn) {
	        var iterator = iteratorFn.call(optValue);
	        var step;
	        if (iteratorFn !== optValue.entries) {
